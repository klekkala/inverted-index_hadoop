...
public class TailSpoutTest extends BaseRichSpout {
	/**
	 * 
	 */
	 ...
	 * 
	 * En este caso se encarga de abrir el archivo del que vamos a leer
	 */
	@Override
	public void open(@SuppressWarnings("rawtypes") Map map,
			TopologyContext ctx, SpoutOutputCollector colector) {
		try {
			this.setCtx(ctx);

			// Creamos un lector de ficheros
			this.lectorFichero = new FileReader("/tmp/test");
	...
	/**
	 * Este método es el que lee cada línea del fichero de entrada y se encarga
	 * de emitirlas para ser procesadas por los bolt
	 */
	@Override
	public void nextTuple() {
		...
		try 
		{
			...
			{
				// Emitimos cada linea por el colector para que los bolts las procesen
